...
export default function LoginPage() {
  ...
  const handleGithubSignIn = async () => {
    ...
    try {
      ...
      if (res?.error) {
        // Fallback for local development when GitHub OAuth app credentials are not yet entered in .env
        ...
        if (demoRes?.ok) {
          ...
          setError(
            "To connect your real GitHub account, please set GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET in .env. You can also sign in with email/password.",
          );
        }
      } else if (res?.url) {
      ...
      // Auto login with demo student in development
      const demoRes = await signIn("credentials", {
        email: "demo@codexa.dev",
        password: "password123",
        redirect: false,
      ...
      if (demoRes?.ok) {
        ...
        setError("GitHub OAuth requires GITHUB_CLIENT_ID & GITHUB_CLIENT_SECRET in .env.");
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-black text-[#E1E0CC]">
      <div className="w-full max-w-md">
        ...
        <p className="text-center font-mono text-xs text-gray-500 mt-6">
          <Link href="/study-hub" className="hover:text-white underline">
            Browse Study Hub as Guest
          </Link>{" "}
          (No account required)
        </p>
      </div>
